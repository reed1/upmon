...
Laravel middleware implementation with Livewire/Filament request decoding. Extends the [Laravel implementation in step 2](../step-2-access-log.md) with handling for Livewire's batch update requests, which would otherwise all log as `POST /livewire/update` with opaque payloads.

See [step-2-access-log.md](../step-2-access-log.md) for the schema and the rules this follows.
...
Same as the [step 2](../step-2-access-log.md#2-sqlite-storage) — no changes needed.
...
class AccessLog
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if (
            ...
            || in_array($request->getPathInfo(), ['/up', '/health', '/health/agent', '/health/frontend-error'])
        ) {
            return $next($request);
        }
...
Same as the [step 2](../step-2-access-log.md#4-register-the-middleware).
